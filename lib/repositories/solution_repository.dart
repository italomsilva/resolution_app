...
class SolutionRepository {
  ...
  Future<List<GetMySolutionsResponseDto>> fetchMySolutions(String token) async {
    final url = Uri.parse('$_baseUrl/solutions/user/app');
    try {
      final response = await http.get(
        url,
        headers: <String, String>{
      ...
      if (response.statusCode == 200) {
        final result = responseData["data"] as List<dynamic>;
        List<GetMySolutionsResponseDto> solutions = result
            .map((e) => GetMySolutionsResponseDto.fromJson(e))
            .toList();
        solutions.sort((a, b) => b.createdAt.compareTo(a.createdAt));
        return solutions;
      } else {
        throw SolutionRepositoryException(
          "Falha ao buscar soluções: Servidor ou Problema de conexão",
  ...
  Future<List<StatsCountSolutionsReactionsResponse>>
  statsCountSolutionsReactions(String token) async {
    final url = Uri.parse('$_baseUrl/solution/stats/solutions-reactions');
    try {
      final response = await http.get(
        url,
        headers: <String, String>{
      ...
      if (response.statusCode == 200) {
        final result = responseData["data"] as List<dynamic>;
        List<StatsCountSolutionsReactionsResponse> solutions = result
            .map((e) => StatsCountSolutionsReactionsResponse.fromJson(e))
            .toList();
        solutions.sort((a, b) => b.createdAt.compareTo(a.createdAt));
        return solutions;
      } else {
        throw SolutionRepositoryException(
          "Falha ao buscar estatisticas de soluções: Servidor ou Problema de conexão",
        );
      }
    } catch (e) {
      return [];
